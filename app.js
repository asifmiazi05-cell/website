const products = [
  {emoji:'🎧', name:'Wireless Earbuds', price:2300, old:2500, tag:'-8%', cat:'audio', brand:'JBL'},
  {emoji:'⌚', name:'Smartwatch Pro', price:3200, old:3400, tag:'-6%', cat:'watch', brand:'Amazfit'},
  {emoji:'🔋', name:'20,000mAh Power Bank', price:2200, old:2450, tag:'-10%', cat:'power', brand:'Anker'},
  {emoji:'🔊', name:'Bluetooth Speaker', price:1984, old:2200, tag:'-10%', cat:'audio', brand:'Xiaomi'},
  {emoji:'🖱️', name:'Gaming Mouse RGB', price:1500, old:null, tag:null, cat:'gaming', brand:'Logitech'},
  {emoji:'💡', name:'Smart LED Strip Light', price:650, old:null, tag:null, cat:'light', brand:'Xiaomi'},
  {emoji:'⌨️', name:'Mechanical Keyboard', price:2800, old:3100, tag:'-9%', cat:'gaming', brand:'Logitech'},
  {emoji:'📷', name:'Webcam 1080p', price:1750, old:null, tag:null, cat:'camera', brand:'Logitech'},
  {emoji:'🔌', name:'65W GaN Charger', price:1400, old:1600, tag:'-13%', cat:'charge', brand:'Ugreen'},
  {emoji:'🧵', name:'USB-C Braided Cable', price:350, old:null, tag:null, cat:'charge', brand:'Anker'},
  {emoji:'📸', name:'Ring Light + Tripod', price:1200, old:1450, tag:'-17%', cat:'camera', brand:'Xiaomi'},
  {emoji:'🎮', name:'Gamepad Controller', price:2100, old:null, tag:null, cat:'gaming', brand:'Sony'},
];
...
function starsHTML(){
  return `<div class="offer-stars">${'★'.repeat(0)}${'☆'.repeat(5)}</div>`;
}
...
function changeQty(name, delta){
  ...
  if(!item) return;
  item.qty += delta;
  const updated = item.qty > 0 ? items : items.filter(i=>i.name!==name);
...
function initOrderForm(){
  ...
  form.addEventListener('submit', function(e){
    ...
    box.innerHTML = `✅ Your order has been received!<br><span class="mono" style="font-size:12.5px; color:var(--text-dim);">${list}</span><br>Subtotal: ${fmtPrice(subtotal)}<br>Delivery Charge: ${fmtPrice(deliveryCharge)}<br>Total: ${fmtPrice(total)}<br>Payment: ${payLine}<br>We will call you shortly to confirm.`;
    box.classList.add('show');
    saveCartItems([]);
    if(typeof renderCheckoutSummary === 'function') renderCheckoutSummary();
  });
